...
class TodoSelectorComponent extends Container implements Focusable {
	...
	private updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i += 1) {
			...
			const line = renderTodoLine(this.theme, todo, isSelected, "  ", this.currentSessionId);
...
class AllTodosSelectorComponent extends Container implements Focusable {
	...
	private updateList(): void {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const display = renderTodoLine(this.theme, todo, isSelected, "    ");
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("todo", {
		description: "Quick todo actions: /todo add <title>, /todo close <id>, /todo reopen <id>",
		getArgumentCompletions: (argumentPrefix: string) => {
			const trimmed = argumentPrefix.trim();

			// Suggest subcommands when empty or partially typed
			if (!trimmed || "add".startsWith(trimmed.toLowerCase()) || "close".startsWith(trimmed.toLowerCase()) || "reopen".startsWith(trimmed.toLowerCase())) {
				const commands = [
					{ value: "add ", label: "add <title>", description: "Create a new todo" },
					{ value: "close ", label: "close <id>", description: "Close a todo" },
					{ value: "reopen ", label: "reopen <id>", description: "Reopen a closed todo" },
				];
				if (!trimmed) return commands;
				return commands.filter((c) => c.label.startsWith(trimmed.toLowerCase()));
			}

			// For close/reopen, suggest todo IDs
			const match = trimmed.match(/^(close|reopen)\s*(.*)/i);
			if (match) {
				const action = match[1].toLowerCase();
				const idPrefix = match[2] || "";
				const todos = listTodosSync(getTodosDir(process.cwd()));
				const relevant = todos.filter((t) =>
					action === "close" ? !isTodoClosed(t.status) : isTodoClosed(t.status),
				);
				const matches = idPrefix ? filterTodos(relevant, idPrefix) : relevant;
				if (!matches.length) return null;
				return matches.map((todo) => ({
					value: `${action} ${formatTodoId(todo.id)}`,
					label: `${formatTodoId(todo.id)} ${todo.title || "(untitled)"}`,
					description: todo.status || "open",
				}));
			}
		...
		handler: async (args, ctx) => {
			...
			if (!trimmed) {
				if (ctx.hasUI) {
					ctx.ui.notify("Usage: /todo add <title> | /todo close <id> | /todo reopen <id>", "info");
				} else {
					console.log("Usage: /todo add <title> | /todo close <id> | /todo reopen <id>");
				}
				return;
			}

			// /todo add <title> [#tag1 #tag2]
			...
			if (addMatch) {
				...
				if (!title) {
					if (ctx.hasUI) {
						ctx.ui.notify("Title required: /todo add <title>", "error");
					} else {
						console.log("Title required: /todo add <title>");
					}
					return;
				}
				...
				if (ctx.hasUI) {
					ctx.ui.notify(`Created ${formatTodoId(id)} "${title}"${tagSuffix}`, "info");
				} else {
					console.log(`Created ${formatTodoId(id)} "${title}"${tagSuffix}`);
				}
				return;
			}

			// /todo close <id>
			const closeMatch = trimmed.match(/^close\s+(.+)/i);
			if (closeMatch) {
				const result = await updateTodoStatus(todosDir, closeMatch[1].trim(), "closed", ctx);
				if ("error" in result) {
					if (ctx.hasUI) ctx.ui.notify(result.error, "error");
					else console.log(result.error);
					return;
				}
				const todo = result as TodoRecord;
				if (ctx.hasUI) ctx.ui.notify(`Closed ${formatTodoId(todo.id)} "${getTodoTitle(todo)}"`, "info");
				else console.log(`Closed ${formatTodoId(todo.id)} "${getTodoTitle(todo)}"`);
				return;
			}

			// /todo reopen <id>
			const reopenMatch = trimmed.match(/^reopen\s+(.+)/i);
			if (reopenMatch) {
				const result = await updateTodoStatus(todosDir, reopenMatch[1].trim(), "open", ctx);
				if ("error" in result) {
					if (ctx.hasUI) ctx.ui.notify(result.error, "error");
					else console.log(result.error);
					return;
				...
				const todo = result as TodoRecord;
				if (ctx.hasUI) ctx.ui.notify(`Reopened ${formatTodoId(todo.id)} "${getTodoTitle(todo)}"`, "info");
				else console.log(`Reopened ${formatTodoId(todo.id)} "${getTodoTitle(todo)}"`);
				return;
			}

			// Unknown subcommand
			if (ctx.hasUI) {
				ctx.ui.notify(`Unknown: /todo ${trimmed}. Try: add, close, reopen`, "error");
			} else {
